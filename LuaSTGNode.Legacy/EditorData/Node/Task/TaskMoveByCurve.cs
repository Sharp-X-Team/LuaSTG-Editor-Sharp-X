...
{
    [Serializable, NodeIcon("taskBeziermovetoex.png")]
    [RequireAncestor(typeof(TaskAlikeTypes))]
    [LeafNode]
    [RCInvoke(3)]
    public class TaskMoveByCurve : TreeNode
    {
        [JsonConstructor]
        public TaskMoveByCurve() : base() { }

        public TaskMoveByCurve(DocumentData workSpaceData) : this(workSpaceData, "60", "Bezier", "MODE_NORMAL", "") { }

        public TaskMoveByCurve(DocumentData workSpaceData, string time, string type, string mode, string pset)
            : base(workSpaceData)
        {
            Time = time;
            Type = type;
        ...
        public override string ToString()
        {
            ...
            return $"Move following curve \"{NonMacrolize(1)}\" in {NonMacrolize(0)} frame(s), with relative anchors"
                + sb.ToString() + mode;
        }
        ...
        {
            var n = new TaskMoveByCurve(parentWorkSpace);
            n.DeepCopyFrom(this);
            return n;
        }
