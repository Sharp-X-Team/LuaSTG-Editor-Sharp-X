...
{
    [Serializable, NodeIcon("render4v3d.png")]
    [RequireAncestor(typeof(RenderAlikeTypes))]
    [LeafNode]
    [CreateInvoke(0)]
    public class Render4V3D : TreeNode
    {
        [JsonConstructor]
        ...
        public override IEnumerable<string> ToLua(int spacing)
        {
            ...
            yield return sp + p1 + $", {Macrolize(1)}\n";
            yield return sp + sn + $", {Macrolize(2)}\n";
            yield return sp + sn + $", {Macrolize(3)}\n";
            yield return sp + sn + $", {Macrolize(4)})\n";
        }

        public override string ToString()
        {
            return $"Render {NonMacrolize(0)} in quadrilateral + z with vertex position "
                + $"({NonMacrolize(1)}), ({NonMacrolize(2)}), ({NonMacrolize(3)}), ({NonMacrolize(4)})";
        }

        public override object Clone()
        {
            var n = new Render4V3D(parentWorkSpace);
            n.DeepCopyFrom(this);
            return n;
        }
