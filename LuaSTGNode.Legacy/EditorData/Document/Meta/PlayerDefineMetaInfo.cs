...
{
    public class PlayerDefineMetaInfo : MetaInfo, IComparable<PlayerDefineMetaInfo>
    {
        PlayerInit Init { get; set; }

        public PlayerDefineMetaInfo(PlayerDefine target) : base(target) { }

        private void TryChild()
        {
            foreach (TreeNode t in this.target.GetLogicalChildren())
            {
                if (t is PlayerInit) Init = t as PlayerInit;
            }
        }
        ...
        public int CompareTo(PlayerDefineMetaInfo other)
        {
        ...
        {
            documentMetaData.aggregatableMetas[(int)MetaType.Player].Add(meta);
        }

        public override void Remove(IAggregatableMeta meta, MetaDataEntity documentMetaData)
        {
            documentMetaData.aggregatableMetas[(int)MetaType.Player].Remove(meta);
        }
        ...
        public override MetaModel GetFullMetaModel()
        {
            ...
            {
                Icon = "/LuaSTGNode.Legacy;component/images/16x16/playerdefine.png",
                Text = Name
            };
