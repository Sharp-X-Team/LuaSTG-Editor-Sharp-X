...
{
    public class PlayerBulletDefineMetaInfo : MetaInfo, IComparable<PlayerBulletDefineMetaInfo>
    {
        PlayerBulletInit Init { get; set; }

        public PlayerBulletDefineMetaInfo(PlayerBulletDefine target) : base(target) { }

        private void TryChild()
        {
            foreach (TreeNode t in this.target.GetLogicalChildren())
            {
                if (t is PlayerBulletInit) Init = t as PlayerBulletInit;
            }
        }
        ...
        public int CompareTo(PlayerBulletDefineMetaInfo other)
        {
        ...
        {
            documentMetaData.aggregatableMetas[(int)MetaType.PlayerBullet].Add(meta);
        }

        public override void Remove(IAggregatableMeta meta, MetaDataEntity documentMetaData)
        {
            documentMetaData.aggregatableMetas[(int)MetaType.PlayerBullet].Remove(meta);
        }
        ...
        public override MetaModel GetFullMetaModel()
        {
            ...
            {
                Icon = "/LuaSTGNode.Legacy;component/images/16x16/playerdefinebullet.png",
                Text = Name
            };
